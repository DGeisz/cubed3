...
pub const SEC_IN_24_HOUR: i64 = 60 * 60 * 24;
...
pub fn calculate_prices(price_ema: u64, mut period_in_sec: u64) -> PricePackage {
    ...
    let mut current = if period_in_sec < (SEC_IN_24_HOUR as u64) {
        price_ema
    } else {
        let milli_supply = 1000 * period_in_sec / (SEC_IN_24_HOUR as u64);

        (price_ema * 1000) / milli_supply
    };

    current = std::cmp::max(min_canvas_price_lamports, current);
    let mut next_target = (current * (SEC_IN_24_HOUR as u64)) / (period_in_sec);
